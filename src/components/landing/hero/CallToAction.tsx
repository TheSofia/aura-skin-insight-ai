...
const CallToAction: React.FC<CallToActionProps> = ({ animationStates, cursorProximity }) => {
  return (
    <>
      ...
      {/* Shop CTA */}
      <RippleButton 
        className={`glass-button hover:bg-beautyagent-light-grey hover:text-beautyagent-dark-grey text-md px-6 py-4 h-auto mb-6 transition-all duration-1200 ease-out transform ${
          animationStates.ctaReady ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'
        } liquid-button`}
        style={{ 
          transitionDelay: '1500ms',
          transform: animationStates.ctaReady ? 
            `translateY(0) scale(${1 + cursorProximity * 0.03})` : 
            'translateY(6px) scale(1)',
          boxShadow: `0 6px 20px -6px rgba(34, 31, 38, ${0.15 + cursorProximity * 0.08})`,
          transition: 'all 0.8s cubic-bezier(0.19, 1, 0.22, 1)'
        }}
        asChild
      >
